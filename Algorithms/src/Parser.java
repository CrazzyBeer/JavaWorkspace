...
public class Parser {
	public static class Token {
		...
		public Token(String s,int is) {
			this.token = s;
			if ( is == 0) isNumber = true;
			else if ( is == 1) isOperator = true;
			else if ( is == 2) isLeftBracket = true;
			else if ( is == 3) isRightBracket = true;
		}
	...
	private static void check(String s) {
		...
		for (int i = 0; i<s.length(); i++) {
			...
			if ( !isNum(c) &&
				 ...
				 c != ' ' &&
				 !isLeftBracket(c) &&
				 !isRightBracket(c) ) throw new  IllegalArgumentException();
		}
	}
	...
	private static Queue <Token> translate(String s) {
		...
		while (!tokens.isEmpty()) {
			...
			tokens.remove(0);
			if (t.isNumber()) output.enqueue(t);
				...
				if (!operators.peek().getToken().equals("(")) throw new IllegalArgumentException();
				operators.pop();
			}
		}
		while (!operators.isEmpty()) {
			Token t = operators.pop();
			if (t.isLeftBracket() || t.isRightBracket()) throw new IllegalArgumentException();
			output.enqueue(t);
		}
	...
	public static BigDecimal eval(String s) {
		...
		while (!output.isEmpty()) {
			...
			if (t.isOperator()) {
				...
				else if ( op.equals("-")) res = n1.subtract(n2);
				else if ( op.equals("*")) res = n1.multiply(n2);
				else if ( op.equals("/")) res = n1.divide(n2);
				else if ( op.equals("^")) {
					res = n1.pow(Integer.parseInt(n2.toString()));
					if (n1.toString().length() > 5000) throw new ArithmeticException();
				}
				numbers.push(res);
				n1 = null;
				n2 = null;
